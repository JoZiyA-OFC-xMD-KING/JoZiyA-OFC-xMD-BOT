...
const yourName = "*POWERED by JoZiyA-OFC-xMD*"; // YOURBOTNAME 💚 කියන තැනට ඔයාගේ බොට්ගේ නම හරි ඔයාගෙ නම හරි දාන්න.
...
await conn.sendMessage(from,{document: {url: data.datalist.list[0].file.path_alt},fileName: data.datalist.list[0].name,mimetype: 'application/vnd.android.package-archive',caption: `> *POWERED by JoZiyA-OFC-xMD*`},{quoted: mek})
...
`
	 ...
	 const mass = await conn.sendMessage(from, { image: { url: `https://files.catbox.moe/96u4ui.jpg`  }, caption: `${pakaya}\n\n` }, { quoted: mek });
...
async(conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/ASITHA-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        let message = {
            header: 'JoZiyA-OFC-xMD SIHALASUB.LK MOVIE SEARCH',
            footer: '> *POWERED by JoZiyA-OFC-xMD*',
...
`;
        ...
        conn.ev.on('messages.upsert', async (messageUpdate) => {
            ...
            if (isReplyToSentMsg) {
                ...
                await conn.sendMessage(from, { react: { text: '⬆️', key: mek.key } });
                if (messageType === '1') {
                    await conn.sendMessage(from, { document: { url: directLink }, mimetype: "video/mp4", fileName: `🎬 JoZiyA-OFC-xMD 🎬\n${sss?.result?.title ?? 'Movie'}.mkv`, caption: `> ${sss?.result?.title ?? 'Movie'}\n\n> 480p\n\n> *POWERED by JoZiyA-OFC-xMD*` }, { quoted: mek });
                } else if (messageType === '2') {
                    await conn.sendMessage(from, { document: { url: directLink1 }, mimetype: "video/mp4", fileName: `🎬 JoZiyA-OFC-xMD 🎬\n${sss?.result?.title ?? 'Movie'}.mkv`, caption: `> ${sss?.result?.title ?? 'Movie'}\n\n> 720p\n\n> *POWERED by JoZiyA-OFC-xMD*` }, { quoted: mek });
                } else if (messageType === '3') {
                    await conn.sendMessage(from, { document: { url: directLink2 }, mimetype: "video/mp4", fileName: `🎬 JoZiyA-OFC-xMD 🎬\n${sss?.result?.title ?? 'Movie'}.mkv`, caption: `> ${sss?.result?.title ?? 'Movie'}\n\n> 1080p\n\n> *POWERED by JoZiyA-OFC-xMD*` }, { quoted: mek });
                }
    ...
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/ASITHA-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        await conn.sendMessage(from, { 
            ...
            caption: `\n\n> *POWERED by JoZiyA-OFC-xMD*`,
            mimetype: "video/mp4",
            fileName: `☠️JoZiyA-OFC-xMD☠️.mp4`
        });
...
const datas = q.split(" & ")[1] 
 await conn.sendMessage(data, { document : { url : m.quoted.msg  } ,caption: `\n${datas}\n\n> *POWERED by JoZiyA-OFC-xMD*`  ,mimetype: "video/mp4" , fileName: `🎬 JoZiyA-OFC-xMD 🎬\n${datas}.mp4` } )
		} catch (e) {
reply('❗ Error' + e )
l(e)
}
...
*➟➟➟➟➟➟➟➟➟➟➟➟➟➟*
▫️🔗 *Join :* https://whatsapp.com/channel/0029VbBBr1iElagw9wIc9K3M
*➟➟➟➟➟➟➟➟➟➟➟➟➟➟*
    ...
    try {
      const msr = (await fetchJson('https://raw.githubusercontent.com/athulakumara604/ASITHA-MD-DATABASE/refs/heads/main/Mreply.json')).replyMsg
        ...
        await conn.sendMessage(from, { 
            ...
            caption: `\n*${titel}*\n\n> *${si}*\n\n> *POWERED by JoZiyA-OFC-xMD*`,
            mimetype: "video/mp4",
            fileName: `🎬 JoZiyA-OFC-xMD 🎬${titel}.mp4`
        });
