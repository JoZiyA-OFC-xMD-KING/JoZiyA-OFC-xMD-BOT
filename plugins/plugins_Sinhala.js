...
// Fetch premium users
        ...
        if (!isPreUser) {
            return reply("🚩 This command is only available to premium users, buy premium 0766002356");
        }
...
let prefix = config.PREFIX;
        ...
        buttonParamsJson: JSON.stringify({
            display_text: "GITHUB",
            url: "https://github.com/JoZiyA-OFC-xMD-KING/JoZiyA-OFC-xMD-BOT",
            merchant_url: "https://github.com/JoZiyA-OFC-xMD-KING/JoZiyA-OFC-xMD-BOT"
        }),}, 
                { name: 'single_select',
            buttonParamsJson: JSON.stringify({
...
let msg = `📌 Select Your Movie`
  let message = {
            header: 'JoZiyA-OFC-xMD MOVIE SEARCH',
...
conn.ev.on('messages.upsert', async (messageUpdate) => {
    ...
    await conn.sendMessage(from, { react: { text: '⬆️', key: mek.key } });
  if (messageType === '1') {
await conn.sendMessage(from, { document : { url : pp2 } ,mimetype: "video/mp4",fileName: `🎬 JoZiyA-OFC-xMD 🎬\n${urll.result.data.title}.mkv`,caption :`> ${urll.result.data.title}\n\n> 480p\n\n> *POWERED by JoZiyA-OFC-xMD*` }, { quoted: mek })
  }else if (messageType === '2') {
    await conn.sendMessage(from, { document : { url : pp1  } ,mimetype: "video/mp4",fileName: `🎬 JoZiyA-OFC-xMD 🎬\n${urll.result.data.title}.mkv`,caption :`> ${urll.result.data.title}\n\n> 720p\n\n> *POWERED by JoZiyA-OFC-xMD*` }, { quoted: mek })
  }else if (messageType === '3'){
await conn.sendMessage(from, { document : { url : pp  } ,mimetype: "video/mp4",fileName: `🎬 JoZiyA-OFC-xMD 🎬\n${urll.result.data.title}.mkv`,caption :`> ${urll.result.data.title}\n\n> 1080p\n\n> *POWERED by JoZiyA-OFC-xMD*` }, { quoted: mek })
}  
